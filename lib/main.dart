...
class SessionManager {
  ...
  // NEW: key used to persist "last time this user opened the group chat".
  static const _kGroupSeen = 'session_groupSeen';
  static Future<void> save({required String role}) async {
    ...
    // NEW: persist group-chat "last seen" so it survives reload/re-login.
    if (AppData.groupChatLastSeen != null) {
      await p.setString(_kGroupSeen, AppData.groupChatLastSeen!.toIso8601String());
    }
  }
  static Future<String?> restore() async {
    ...
    AppData.currentUserAvatarColor = p.getInt(_kAvatarColor)   ?? 0xFFD4A89A;
    AppData.currentUserIsAdmin      = role == 'warden';
    AppData.currentUserIsSuperAdmin = role == 'superadmin';
    // NEW: restore group-chat "last seen" — this is the actual bug fix.
    // Without this, AppData.groupChatLastSeen was always null after every
    // re-login/page reload, which made the unread listeners below treat
    // every historical group message as "new".
    final seenStr = p.getString(_kGroupSeen);
    AppData.groupChatLastSeen = seenStr != null ? DateTime.tryParse(seenStr) : null;
    return role;
  }
...
class DemoBanner extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    if (!AppData.isDemoAccount) return SizedBox.shrink();
    return Container(
      width: double.infinity,
      padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [Color(0xFF7B2FBE), Color(0xFF5C3D99)]),
      ),
      child: Row(children: [
        Icon(Icons.play_circle_outline, color: Colors.white, size: 16),
        SizedBox(width: 8),
        Expanded(child: Text('Demo Mode — View only. Some actions are disabled.',
            style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500))),
        GestureDetector(
          onTap: () async {
            AppData.logout();
            Navigator.pushAndRemoveUntil(context,
                MaterialPageRoute(builder: (_) => WelcomeScreen()), (_) => false);
          },
          child: Text('Exit', style: TextStyle(color: Colors.white, fontSize: 12,
...
class AppData {
  ...
  static bool   isDemoAccount = false; // true = demo mode, all writes blocked
  ...
  static int    currentUserAvatarColor = 0xFFD4A89A; // default purple
  ...
  static const List<int> avatarColors = [
    0xFFD4A89A, 0xFFE91E63, 0xFF9C27B0, 0xFF3F51B5,
  ...
  // NEW: now persists immediately, so "last seen" survives a re-login or a
  // browser refresh instead of resetting to null every time.
  static Future<void> markGroupChatSeen() async {
    groupChatLastSeen = DateTime.now();
    groupUnreadCount  = 0;
    try {
      final p = await SharedPreferences.getInstance();
      await p.setString('session_groupSeen', groupChatLastSeen!.toIso8601String());
    } catch (_) {}
  }

  static void logout() {
    currentUserName = currentUserRoom = currentUserEmail = currentUserHostelId = selectedHostelId = selectedHostelName = currentUserUid = '';
    currentUserIsAdmin = currentUserIsSuperAdmin = isDemoAccount = false;
    currentUserEmoji = ''; currentUserAvatarColor = 0xFFD4A89A;
    myRequests.clear(); myLendings.clear();
    groupChatLastSeen = null; groupUnreadCount = 0;
    FirebaseService.signOut();
    SessionManager.clear(); // also wipes the persisted "group seen" timestamp
  }
}
  ...
  Widget build(BuildContext context) {
    return MaterialApp(title: 'HostelLink', debugShowCheckedModeBanner: false,
        theme: ThemeData(
          ...
          primaryColor: Color(0xFFD4A89A),
          scaffoldBackgroundColor: Color(0xFF0D0D0D),
          colorScheme: ColorScheme.dark(
            primary: Color(0xFFD4A89A),
            secondary: Color(0xFFE0BDB0),
          ...
          bottomNavigationBarTheme: BottomNavigationBarThemeData(
            backgroundColor: Color(0xFF0D0D0D),
            selectedItemColor: Color(0xFFD4A89A),
            unselectedItemColor: Colors.white38,
          ),
          tabBarTheme: TabBarThemeData(
            labelColor: Color(0xFFD4A89A),
            unselectedLabelColor: Colors.white38,
            indicatorColor: Color(0xFFD4A89A),
          ),
          inputDecorationTheme: InputDecorationTheme(
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Color(0xFFD4A89A), width: 1.5)),
          ),
          textTheme: TextTheme(
          ...
          dropdownMenuTheme: DropdownMenuThemeData(
            menuStyle: MenuStyle(backgroundColor: WidgetStateProperty.all(Color(0xFF1A1A2E))),
          ),
        ),
        home: _homeForRole());
  }
...
// WELCOME SCREEN
// ═══════════════════════════════════════════════════════════════════
void showDemoBlockedMessage(BuildContext context) {
  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
    content: Row(children: [
      Icon(Icons.block, color: Colors.white, size: 18),
      SizedBox(width: 10),
      Expanded(child: Text('Demo accounts are view-only. This action is disabled.')),
    ]),
...
class WelcomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      body: SingleChildScrollView(child: Padding(padding: EdgeInsets.fromLTRB(24, 60, 24, 40), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        // Logo / icon
        Container(
          ...
          decoration: BoxDecoration(
            shape: BoxShape.circle,
            color: Color(0xFF1A1A2E),
            border: Border.all(color: Color(0xFFD4A89A).withValues(alpha: 0.4), width: 2),
            boxShadow: [BoxShadow(color: Color(0xFFD4A89A).withValues(alpha: 0.2), blurRadius: 24, spreadRadius: 4)],
          ),
          child: Icon(Icons.home_outlined, size: 52, color: Color(0xFFD4A89A)),
        ),
        SizedBox(height: 28),
        Text('HostelLink', style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Colors.white, letterSpacing: 1)),
        SizedBox(height: 8),
        Text('Share. Borrow. Connect.', style: TextStyle(fontSize: 15, color: Colors.white54, letterSpacing: 0.5)),
        SizedBox(height: 32),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: Color(0xFFD4A89A).withValues(alpha: 0.25)),
          ),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Icon(Icons.play_circle_outline, color: Color(0xFFD4A89A), size: 18),
              SizedBox(width: 8),
              Text('DEMO ACCOUNTS', style: TextStyle(color: Color(0xFFD4A89A),
                  fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 2)),
            ]),
            SizedBox(height: 4),
            Text('Explore the app instantly — read-only preview',
                style: TextStyle(color: Colors.white38, fontSize: 12)),
            SizedBox(height: 14),
            _DemoCard(
              icon: Icons.school_outlined,
              label: 'Demo Student',
              subtitle: 'Browse borrow, market & chat',
              color: Color(0xFF7B2FBE),
              onTap: () {
                AppData.isDemoAccount = true;
                AppData.selectedHostelId   = 'lcwu_lahore';
                AppData.selectedHostelName = 'LCWU Girls Hostel';
                AppData.currentUserEmail       = 'demo.student@hostellink.pk';
                AppData.currentUserName        = 'Demo Student';
                AppData.currentUserRoom        = 'Room 1 • Block A';
                AppData.currentUserHostelId    = 'lcwu_lahore';
                AppData.currentUserEmoji       = '🎓';
                AppData.currentUserAvatarColor = 0xFF7B2FBE;
                AppData.currentUserUid         = 'demo_student_uid';
            ...
            _DemoCard(
              ...
              label: 'Demo Warden',
              subtitle: 'Explore the warden dashboard',
              color: Color(0xFFB71C1C),
              onTap: () {
                AppData.isDemoAccount = true;
                AppData.selectedHostelId   = 'lcwu_lahore';
                AppData.selectedHostelName = 'LCWU Girls Hostel';
                AppData.currentUserEmail       = 'demo.warden@hostellink.pk';
                AppData.currentUserName        = 'Demo Warden';
                AppData.currentUserHostelId    = 'lcwu_lahore';
                AppData.currentUserIsAdmin     = true;
                AppData.currentUserIsSuperAdmin = false;
                AppData.currentUserEmoji       = '🛡️';
                AppData.currentUserAvatarColor = 0xFFB71C1C;
                AppData.currentUserUid         = 'demo_warden_uid';
                Navigator.pushReplacement(context,
            ...
            _DemoCard(
              ...
              subtitle: 'See hostel & warden management',
              color: Color(0xFFF59E0B),
              onTap: () {
                AppData.isDemoAccount = true;
                AppData.currentUserEmail        = 'demo.admin@hostellink.pk';
                AppData.currentUserName         = 'Demo Admin';
                AppData.currentUserIsAdmin      = false;
                AppData.currentUserIsSuperAdmin = true;
                AppData.currentUserUid          = 'demo_admin_uid';
                Navigator.pushReplacement(context,
                    MaterialPageRoute(builder: (_) => SuperAdminPanelScreen()));
              },
            ),
          ]),
        ),
        ...
        Row(children: [
          ...
          Padding(padding: EdgeInsets.symmetric(horizontal: 12),
              child: Text('OR LOGIN', style: TextStyle(color: Colors.white24, fontSize: 11, letterSpacing: 1))),
          Expanded(child: Divider(color: Colors.white12)),
        ]),
        SizedBox(height: 20),
        ...
        SizedBox(width: double.infinity, child: ElevatedButton.icon(
          onPressed: () => Navigator.push(context,
              MaterialPageRoute(builder: (_) => SelectHostelScreen())),
          icon: Icon(Icons.apartment_outlined),
          label: Text('Select Your Hostel',
              style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)),
          style: ElevatedButton.styleFrom(
            backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C),
        ...
        SizedBox(width: double.infinity, child: OutlinedButton.icon(
          ...
          label: Text('Warden Login', style: TextStyle(fontSize: 15)),
...
class _DemoCard extends StatelessWidget {
  ...
  const _DemoCard({required this.icon, required this.label, required this.subtitle,
...
class _SelectHostelScreenState extends State<SelectHostelScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Select Your Hostel'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: Column(children: [
        Container(width: double.infinity, padding: EdgeInsets.all(14), color: Color(0xFFD4A89A).withValues(alpha: 0.07),
            child: Row(children: [Icon(Icons.lock_outline, color: Color(0xFFD4A89A)), SizedBox(width: 10), Expanded(child: Text('Registration requires warden approval. Each hostel\'s data is completely private.', style: TextStyle(color: Colors.white54, fontSize: 13)))])),
        Padding(padding: EdgeInsets.fromLTRB(16,16,16,8), child: DropdownButtonFormField<String>(initialValue: _city,
        ...
        Expanded(child: ListView.builder(padding: EdgeInsets.symmetric(horizontal: 16), itemCount: list.length, itemBuilder: (ctx, i) {
          ...
          return Card(margin: EdgeInsets.only(bottom: 10), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: sel ? Color(0xFFD4A89A) : Colors.white12, width: sel ? 2 : 1)),
              child: ListTile(contentPadding: EdgeInsets.all(12),
                  leading: CircleAvatar(backgroundColor: sel ? Color(0xFFD4A89A) : Color(0xFFD4A89A).withValues(alpha: 0.1), child: Icon(Icons.apartment, color: sel ? Color(0xFF1A0F0C) : Color(0xFFD4A89A))),
                  title: Text(h['name']!, style: TextStyle(fontWeight: FontWeight.bold, color: sel ? Color(0xFFD4A89A) : Colors.white)),
                  subtitle: Text('${h['university']}\n📍 ${h['city']}', style: TextStyle(fontSize: 12, color: Colors.white54)),
                  trailing: Icon(sel ? Icons.check_circle : Icons.radio_button_unchecked, color: sel ? Color(0xFFD4A89A) : Colors.white38),
        ...
        Padding(padding: EdgeInsets.all(16), child: SizedBox(width: double.infinity, child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C),
              padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), disabledBackgroundColor: Colors.white12, disabledForegroundColor: Colors.white38),
...
class AuthChoiceScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white, elevation: 0),
      body: SingleChildScrollView(child: Padding(padding: EdgeInsets.fromLTRB(24, 60, 24, 40), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(
          width: 80, height: 80,
          decoration: BoxDecoration(
            shape: BoxShape.circle, color: Color(0xFF1A1A2E),
            border: Border.all(color: Color(0xFFD4A89A).withValues(alpha: 0.4), width: 1.5),
          ),
          child: Icon(Icons.apartment_outlined, size: 40, color: Color(0xFFD4A89A)),
        ),
        SizedBox(height: 20),
        ...
        SizedBox(height: 10),
        Container(padding: EdgeInsets.symmetric(horizontal: 14, vertical: 6),
            decoration: BoxDecoration(color: Color(0xFFD4A89A).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(20), border: Border.all(color: Color(0xFFD4A89A).withValues(alpha: 0.3))),
            child: Text('🔐 Warden-Approved Registration', style: TextStyle(color: Color(0xFFD4A89A), fontSize: 13))),
        SizedBox(height: 44),
        SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => LoginScreen())),
            style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
            child: Text('Login to My Account', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)))),
        SizedBox(height: 14),
        SizedBox(width: double.infinity, child: OutlinedButton(
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => RegisterScreen())),
            style: OutlinedButton.styleFrom(foregroundColor: Colors.white70, side: BorderSide(color: Colors.white24), padding: EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
            child: Text('Request Registration', style: TextStyle(fontSize: 15)))),
        SizedBox(height: 12),
        TextButton.icon(
          onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CheckRequestStatusScreen())),
          icon: Icon(Icons.search, color: Colors.white38, size: 16),
          label: Text('Check my request status', style: TextStyle(color: Colors.white38, fontSize: 13)),
        ),
        SizedBox(height: 8),
        TextButton(onPressed: () => Navigator.pop(context), child: Text('← Choose different hostel', style: TextStyle(color: Colors.white38))),
      ]))),
    );
  }
}
...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void _submitRequest() async {
    ...
    if (submitError != null) {
      showDialog(context: context, builder: (_) => AlertDialog(
        ...
        content: Text(submitError),
        actions: [ElevatedButton(onPressed: () => Navigator.pop(context),
            style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C)), child: Text('OK'))],
      ));
    } else {
      ...
      showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 8),
            Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: Color(0xFFD4A89A).withValues(alpha: 0.08), borderRadius: BorderRadius.circular(10)),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Request Registration'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(children: [
        // Info banner
        Container(padding: EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.blue.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.blue.withValues(alpha: 0.3))),
            child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Icon(Icons.info_outline, color: Colors.blue, size: 22), SizedBox(width: 10),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('How registration works:', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue[200])),
                SizedBox(height: 4),
                Text('Fill this form → Warden receives your request → Warden approves → You can login!', style: TextStyle(color: Colors.blue[100], fontSize: 13)),
              ])),
            ])),
        SizedBox(height: 20),
        TextField(controller: nameCtrl, style: TextStyle(color: Colors.white), decoration: InputDecoration(labelText: 'Full Name', labelStyle: TextStyle(color: Colors.white54), prefixIcon: Icon(Icons.person_outline, color: Color(0xFFCC88FF)), filled: true, fillColor: Color(0xFF1A1A2E), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Color(0xFFD4A89A), width: 1.5)))),
        SizedBox(height: 14),
        TextField(controller: emailCtrl, keyboardType: TextInputType.emailAddress, style: TextStyle(color: Colors.white), decoration: InputDecoration(labelText: 'Email Address', labelStyle: TextStyle(color: Colors.white54), prefixIcon: Icon(Icons.email_outlined, color: Color(0xFFCC88FF)), filled: true, fillColor: Color(0xFF1A1A2E), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Color(0xFFD4A89A), width: 1.5)))),
        SizedBox(height: 14),
        TextField(controller: roomCtrl, style: TextStyle(color: Colors.white), decoration: InputDecoration(labelText: 'Room Number', labelStyle: TextStyle(color: Colors.white54), prefixIcon: Icon(Icons.door_back_door_outlined, color: Color(0xFFCC88FF)), filled: true, fillColor: Color(0xFF1A1A2E), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Color(0xFFD4A89A), width: 1.5)))),
        SizedBox(height: 14),
        DropdownButtonFormField<String>(initialValue: _block,
        ...
        SizedBox(width: double.infinity, child: ElevatedButton.icon(
          ...
          label: _loading ? SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Color(0xFF1A0F0C), strokeWidth: 2)) : Text('Send Registration Request', style: TextStyle(fontSize: 16)),
          style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C),
...
class _CheckRequestStatusScreenState extends State<CheckRequestStatusScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Check Request Status'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: Padding(padding: EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [
          ...
          ElevatedButton(onPressed: _check,
              style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(horizontal: 16, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
    ...
    return Card(
      ...
      child: Padding(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (r['status'] == 'rejected' && (r['rejectionReason']?.toString() ?? '').isNotEmpty) ...[
          SizedBox(height: 8),
          Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.red.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(10)),
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(r['rejectionReason']?.toString() ?? '', style: TextStyle(color: Colors.red[300])),
                ])),
              ])),
        ],
        if (r['status'] == 'approved') ...[
          ...
          SizedBox(width: double.infinity, child: ElevatedButton.icon(
            ...
            icon: Icon(Icons.login), label: Text('Go to Login'),
            style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          )),
        ],
        if (r['status'] == 'pending') ...[
          SizedBox(height: 12),
          Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.orange.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(10)),
              child: Text('Your request is waiting for the warden to review it.\nPlease check back later or contact your warden.', style: TextStyle(color: Colors.orange[200], fontSize: 13))),
        ],
      ])),
    );
  }
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _forgotPassword() {
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      title: Row(children: [Icon(Icons.lock_reset, color: Color(0xFFD4A89A)), SizedBox(width: 8), Text('Reset Password')]),
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Enter your registered email address. We\'ll send you a link to reset your password.', style: TextStyle(color: Colors.white60, fontSize: 13)),
        SizedBox(height: 14),
        ...
        ElevatedButton(
          ...
          onPressed: () async {
            ...
            showDialog(context: context, builder: (_) => AlertDialog(
              ...
              actions: [ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C)), child: Text('OK'))],
            ));
          },
  ...
  void _login() async {
    ...
    AppData.currentUserAvatarColor  = int.tryParse(userData['avatarColor'] ?? '0xFFD4A89A') ?? 0xFFD4A89A;
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Login — ${AppData.selectedHostelName}'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: Column(children: [
        NoInternetBanner(),
        Expanded(child: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(children: [
          ...
          Container(
            width: 80, height: 80,
            decoration: BoxDecoration(shape: BoxShape.circle, color: Color(0xFF1A1A2E),
              border: Border.all(color: Color(0xFFD4A89A).withValues(alpha: 0.35), width: 1.5),
            ),
            child: Icon(Icons.lock_outline, size: 40, color: Color(0xFFD4A89A)),
          ),
          SizedBox(height: 24),
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _loading ? null : _login,
              style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
              child: _loading ? SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Color(0xFF1A0F0C), strokeWidth: 2.5)) : Text('Sign In', style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold)))),
          SizedBox(height: 4),
          Align(alignment: Alignment.centerRight, child: TextButton(onPressed: _forgotPassword, child: Text('Forgot Password?', style: TextStyle(color: Color(0xFFD4A89A))))),
          TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => RegisterScreen())), child: Text("Don't have an account? Request Registration", style: TextStyle(color: Color(0xFFD4A89A)))),
          TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CheckRequestStatusScreen())), child: Text('Check my request status', style: TextStyle(color: Colors.white38))),
        ]))),
      ]),
    );
  }
...
class _AdminPanelScreenState extends State<AdminPanelScreen> with SingleTickerProviderStateMixin {
  ...
  int _privateUnread = 0; // last known private-chat unread total
  ...
  void _listenChatUnread() {
    ...
    _privateSub = FirebaseService.getPrivateChatsStream(email).listen((snap) {
      ...
      _privateUnread = total;
      if (mounted) setState(() => _chatUnread = _privateUnread + AppData.groupUnreadCount);
    });

    // Group chat unread
    _groupSub = FirebaseService.getGroupMessagesStream(hostelId).listen((snap) {
      // FIX: if this is the very first time ever (lastSeen still null —
      // e.g. brand new session before SessionManager.restore() had anything
      // to load), treat "now" as the seen-point instead of flagging every
      // historical message as unread. markGroupChatSeen() also persists
      // this immediately so a refresh/re-login won't reset it again.
      if (AppData.groupChatLastSeen == null) {
      ...
      builder: (ctx, snap) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white,
          ...
          body: TabBarView(controller: _tabs, children: [
            ...
            snap.connectionState == ConnectionState.waiting
              ...
              return Card(
                ...
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14), side: BorderSide(color: isPending ? Colors.orange.withValues(alpha: 0.4) : Colors.transparent, width: 1.5)),
                child: Padding(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    CircleAvatar(radius: 22, backgroundColor: Color(0xFFD4A89A), child: Text(r['name'].toString()[0].toUpperCase(), style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18))),
                    SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Row(children: [
                      Expanded(child: ElevatedButton.icon(
                        onPressed: _approvingIds.contains(r['id'].toString()) ? null : () async {
                          if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; }
                          ...
                          // NEW: real notification instead of nothing happening.
                          final studentEmail = r['email']?.toString() ?? '';
                          if (err == null && studentEmail.isNotEmpty) {
                            await FirebaseService.addNotification(
                              forEmail: studentEmail,
                              message: 'Your registration request was approved! You can now log in.',
                            );
                          }
                          if (mounted) {
                        ...
                        icon: _approvingIds.contains(r['id'].toString())
                            ? SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: Color(0xFF1A0F0C), strokeWidth: 2))
                            : Icon(Icons.check, size: 18),
                        label: Text(_approvingIds.contains(r['id'].toString()) ? 'Approving...' : 'Approve'),
                        style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                      )),
                      SizedBox(width: 10),
                      Expanded(child: OutlinedButton.icon(
                        onPressed: () => _showRejectDialog(r['id'].toString(), r['email']?.toString() ?? ''),
            ...
            FutureBuilder<List<Map<String, dynamic>>>(
                ...
                builder: (ctx, snap) {
                  if (snap.connectionState == ConnectionState.waiting)
                    return Center(child: CircularProgressIndicator(color: Color(0xFFD4A89A)));
                  ...
                  return StatefulBuilder(builder: (ctx2, setLocal) => ListView.builder(padding: EdgeInsets.all(12), itemCount: students.length, itemBuilder: (ctx, i) {
                    ...
                    return Card(margin: EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                        child: ListTile(
                          leading: CircleAvatar(backgroundColor: Color(0xFFD4A89A), child: Text(u['name'].toString()[0].toUpperCase(), style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
                          title: Text(u['name'].toString(), style: TextStyle(fontWeight: FontWeight.bold)),
                          subtitle: Text('${u['email']}\nRoom: ${u['room']}'),
                          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                            IconButton(
                                icon: Icon(Icons.chat_bubble_outline, color: Color(0xFFD4A89A)),
            ...
            SingleChildScrollView(padding: EdgeInsets.all(20), child: Column(children: [
              ...
              SizedBox(height: 12),
              _statCard('Registered Students', '$_studentCount', Icons.people, Color(0xFFD4A89A)),
              SizedBox(height: 12),
            ...
            // ── TAB 7: Chat ───────────────────────────────────────────
            WardenChatTab(onUnreadChanged: () { if (mounted) setState(() => _chatUnread = _privateUnread + AppData.groupUnreadCount); }),
          ]),
        );
      },
    );
  ...
  void _showRejectDialog(String reqId, String studentEmail) {
    if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; }
        ...
        ElevatedButton(
          onPressed: () async {
            ...
            if (studentEmail.isNotEmpty) {
              await FirebaseService.addNotification(
                forEmail: studentEmail,
                message: reasonCtrl.text.trim().isEmpty
                    ? 'Your registration request was rejected.'
                    : 'Your registration request was rejected. Reason: ${reasonCtrl.text.trim()}',
              );
            }
            Navigator.pop(context);
            setState(() {});
  ...
  Widget _statCard(String label, String val, IconData icon, Color color) => Card(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
      child: Padding(padding: EdgeInsets.all(14), child: Row(children: [
        CircleAvatar(backgroundColor: color.withValues(alpha: 0.1), radius: 22, child: Icon(icon, color: color, size: 22)),
        SizedBox(width: 12),
        ...
        builder: (ctx, snap) {
          ...
          if (items.isEmpty)
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.swap_horiz, size: 60, color: Colors.white24),
              SizedBox(height: 10),
            ...
            itemBuilder: (ctx, i) {
              ...
              return Card(
                ...
                child: Padding(padding: EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    CircleAvatar(
                      backgroundColor: taken ? Colors.green.withValues(alpha: 0.1) : Color(0xFFD4A89A).withValues(alpha: 0.1),
                      radius: 20,
                      child: Icon(taken ? Icons.check_circle_outline : Icons.swap_horiz,
                          color: taken ? Colors.green : Color(0xFFD4A89A), size: 20),
                    ),
                    SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(r['item'].toString(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                      Text('${r['person']}  •  ${r['room']}', style: TextStyle(color: Colors.white54, fontSize: 13)),
                    ])),
                    Container(
                      padding: EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                      decoration: BoxDecoration(
                        color: taken ? Colors.green.withValues(alpha: 0.1) : Colors.orange.withValues(alpha: 0.1),
                  ...
                  Wrap(spacing: 8, children: [
                    ...
                    _wChip(Icons.category_outlined, r['category'].toString(), Color(0xFFD4A89A)),
                  ...
                  if (taken && r['lenderName'] != null) ...[
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.07), borderRadius: BorderRadius.circular(8)),
                      child: Row(children: [
    ...
    return Column(children: [
      ...
      Expanded(child: StreamBuilder(
        ...
        builder: (ctx, snap) {
          ...
          if (lendings.isEmpty)
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.volunteer_activism, size: 60, color: Colors.white24),
              SizedBox(height: 10),
          ...
          return Column(children: [
            Padding(
              ...
              child: Row(children: [
                _summaryChip('${docs.length} Total', Color(0xFFD4A89A)),
                SizedBox(width: 8),
                ...
                return Card(
                  ...
                  child: Padding(padding: EdgeInsets.all(14), child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      SizedBox(height: 3),
                      Text('From: ${l['lenderName'] ?? '—'}', style: TextStyle(color: Colors.white60, fontSize: 12)),
                      Text('To: ${l['lentTo'] ?? '—'}  (${l['room'] ?? ''})', style: TextStyle(color: Colors.white60, fontSize: 12)),
    ...
    return Column(children: [
      ...
      Expanded(child: StreamBuilder(
        ...
        builder: (ctx, snap) {
          ...
          if (listings.isEmpty)
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.storefront_outlined, size: 60, color: Colors.white24),
              SizedBox(height: 10),
                ...
                return Card(
                  ...
                  child: Padding(padding: EdgeInsets.all(14), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                      width: 48, height: 48,
                      decoration: BoxDecoration(
                        color: sold ? Colors.grey.withValues(alpha: 0.1) : Color(0xFFD4A89A).withValues(alpha: 0.08),
                        borderRadius: BorderRadius.circular(12),
                      ),
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(l['title'].toString(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14,
                          color: sold ? Colors.grey : Colors.white)),
                      SizedBox(height: 3),
                      Text('Rs. ${l['price']}  •  ${l['condition']}',
                          style: TextStyle(color: Color(0xFFD4A89A), fontWeight: FontWeight.w600, fontSize: 13)),
                    ...
                    Container(
                      padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                      decoration: BoxDecoration(
                        color: sold ? Colors.grey.withValues(alpha: 0.1) : Colors.green.withValues(alpha: 0.1),
                        borderRadius: BorderRadius.circular(20),
...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Warden Login'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(children: [
        SizedBox(height: 20), Icon(Icons.admin_panel_settings, size: 80, color: Color(0xFFD4A89A)),
        Text('Warden Portal', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFFD4A89A))),
        Text('Account created by HostelLink Super Admin', style: TextStyle(color: Colors.grey, fontSize: 12)),
        SizedBox(height: 30),
        DropdownButtonFormField<String>(initialValue: _selectedHostelId,
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _loading ? null : _login,
            style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
            child: _loading ? SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Color(0xFF1A0F0C), strokeWidth: 2.5)) : Text('Login as Warden', style: TextStyle(fontSize: 18)))),
      ])),
    );
  }
}
...
class _WardenChangePasswordScreenState extends State<WardenChangePasswordScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Color(0xFF0D0D0D),
        appBar: AppBar(title: Text('Set Your Password'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white, automaticallyImplyLeading: false),
        body: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(children: [
          ...
          SizedBox(height: 8),
          Container(padding: EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.orange.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.orange)),
              child: Row(children: [Icon(Icons.warning_amber, color: Colors.orange), SizedBox(width: 8), Expanded(child: Text('You must set a personal password before continuing.', style: TextStyle(color: Colors.orange[200])))])),
          SizedBox(height: 24),
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _loading ? null : _change,
              style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
              child: _loading ? SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Color(0xFF1A0F0C), strokeWidth: 2.5)) : Text('Set Password & Continue', style: TextStyle(fontSize: 16)))),
        ])));
  }
...
class _SuperAdminLoginScreenState extends State<SuperAdminLoginScreen> {
  ...
  void _login() async {
    ...
    setState(() => _loading = false);
    if (error != null) { showDialog(context: context, builder: (_) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), title: Row(children: [Icon(Icons.block, color: Colors.red), SizedBox(width: 8), Text('Access Denied')]), content: Text(error), actions: [ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), child: Text('OK'))])); }
  ...
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: Color(0xFF0D0D0D),
        appBar: AppBar(title: Text('Restricted Access'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white, elevation: 0),
        body: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(children: [
          SizedBox(height: 20), Icon(Icons.shield_outlined, size: 80, color: Color(0xFFD4A89A)),
          ...
          SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _loading ? null : _login,
              style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
              child: _loading ? CircularProgressIndicator(color: Color(0xFF1A0F0C)) : Text('Access Super Admin Panel', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)))),
          SizedBox(height: 20),
        ])));
  }
  Widget _darkField(TextEditingController c, String label, IconData icon, {bool obscure = false, VoidCallback? toggle}) =>
      TextField(controller: c, obscureText: obscure, style: TextStyle(color: Colors.white),
          decoration: InputDecoration(labelText: label, labelStyle: TextStyle(color: Colors.grey), prefixIcon: Icon(icon, color: Color(0xFFD4A89A)),
              ...
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Colors.white12)),
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: Color(0xFFD4A89A)))));
}
...
class _SuperAdminPanelScreenState extends State<SuperAdminPanelScreen> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Super Admin Panel', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), Text('HostelLink Master Control', style: TextStyle(fontSize: 11, color: Color(0xFFD4A89A)))]),
        backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white,
        actions: [IconButton(icon: Icon(Icons.logout), onPressed: () { () async { await SessionManager.clear(); AppData.logout(); Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => WelcomeScreen())); }(); })],
        bottom: TabBar(controller: _tabs, labelColor: Color(0xFFD4A89A), unselectedLabelColor: Colors.white38, indicatorColor: Color(0xFFD4A89A), tabs: [Tab(icon: Icon(Icons.dashboard), text: 'Stats'), Tab(icon: Icon(Icons.apartment), text: 'Hostels'), Tab(icon: Icon(Icons.manage_accounts), text: 'Wardens'), Tab(icon: Icon(Icons.people), text: 'Students')]),
      ),
...
class _StatsTab extends StatelessWidget {
    ...
    return FutureBuilder<List<Map<String,dynamic>>>(
      ...
      builder: (ctx, snap) {
        ...
        return SingleChildScrollView(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(height: 12),
          Row(children: [
            Expanded(child: _sc('Total Students',   '${students.length}', Icons.people, Color(0xFFD4A89A))),
            SizedBox(width: 12),
            ...
            return Card(margin: EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                child: Padding(padding: EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    ...
                    _chip('$s Students', Color(0xFFD4A89A)), SizedBox(width: 8),
  ...
  Widget _chip(String l, Color c) => Container(padding: EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: c.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)), child: Text(l, style: TextStyle(fontSize: 11, color: c, fontWeight: FontWeight.bold)));
}
...
class __HostelsTabState extends State<_HostelsTab> {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...HostelRegistry.hostels.map((h) { bool a = h['active'] == 'yes'; return Card(margin: EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: ListTile(leading: CircleAvatar(backgroundColor: a ? Colors.green.withValues(alpha: 0.12) : Colors.white.withValues(alpha: 0.06), child: Icon(Icons.apartment, color: a ? Colors.green : Colors.white38)), title: Text(h['name']!, style: TextStyle(fontWeight: FontWeight.bold)), subtitle: Text('${h['city']} • ${h['university']}', style: TextStyle(fontSize: 12)), trailing: Switch(value: a, activeColor: Colors.green, onChanged: (_) { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; } setState(() { h['active'] = a ? 'no' : 'yes'; }); FirebaseService.setHostelActive(h['id']!, !a); widget.onRefresh(); }))); }).toList(),
    ]));
  }
}
...
class __WardenTabState extends State<_WardenTab> {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      DropdownButtonFormField<String>(initialValue: _hid, decoration: InputDecoration(labelText: 'Select Hostel', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12))), items: HostelRegistry.activeHostels.map((h) => DropdownMenuItem(value: h['id'], child: Text(h['name']!))).toList(), onChanged: (v) => setState(() => _hid = v)),
      SizedBox(height: 10),
      ...
      SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: _create, icon: Icon(Icons.person_add), label: Text('Create Warden'), style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))),
      ...
      ...AppData.wardenAccounts.map((w) { bool a = w['active'] == 'yes'; bool t = w['isTemp'] == 'yes'; return Card(margin: EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: ListTile(leading: CircleAvatar(backgroundColor: a ? Color(0xFFD4A89A).withValues(alpha: 0.1) : Colors.white.withValues(alpha: 0.06), child: Icon(Icons.manage_accounts, color: a ? Color(0xFFD4A89A) : Colors.white38)), title: Text(w['name']!, style: TextStyle(fontWeight: FontWeight.bold)), subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(w['email']!), Text(HostelRegistry.getHostelName(w['hostelId']!), style: TextStyle(fontSize: 11)), if (t) Text('⚠️ Has not logged in yet', style: TextStyle(color: Colors.orange, fontSize: 11)), if (!t && a) Text('✅ Active', style: TextStyle(color: Colors.green, fontSize: 11))]), trailing: Switch(value: a, activeColor: Colors.green, onChanged: (_) { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; } setState(() { AppData.wardenAccounts[AppData.wardenAccounts.indexOf(w)]['active'] = a ? 'no' : 'yes'; }); }))); }).toList(),
    ]));
  }
}
        ...
        return ListView.builder(
          ...
          itemBuilder: (ctx, i) {
            ...
            return Card(
              ...
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
              child: ListTile(
                leading: CircleAvatar(backgroundColor: Color(0xFFD4A89A),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _listenUnread() {
    ...
    _groupSub = FirebaseService.getGroupMessagesStream(hostelId).listen((snap) {
      // FIX: same issue as the warden dashboard — a null lastSeen used to
      // mean "flag everything as unread". Now it just means "seen as of
      // right now", and that gets persisted immediately.
      if (AppData.groupChatLastSeen == null) {
        AppData.markGroupChatSeen();
      }
      final lastSeen = AppData.groupChatLastSeen;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white, automaticallyImplyLeading: false,
        leading: Padding(
        ...
        actions: [
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseService.getNotificationsStream(AppData.currentUserEmail),
            builder: (ctx, snap) {
              final count = snap.data?.docs.length ?? 0;
              return Stack(children: [
                IconButton(icon: Icon(Icons.notifications_outlined), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => NotificationsScreen()))),
                if (count > 0) Positioned(right: 8, top: 8, child: Container(width: 16, height: 16, decoration: BoxDecoration(color: Colors.red, shape: BoxShape.circle), child: Center(child: Text('$count', style: TextStyle(color: Colors.white, fontSize: 10))))),
              ]);
            },
          ),
        ],
      ...
      floatingActionButton: _tab == 0
          ? FloatingActionButton(backgroundColor: Color(0xFFD4A89A), child: Icon(Icons.add, color: Color(0xFF1A0F0C)),
          onPressed: () async { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; } await Navigator.push(context, MaterialPageRoute(builder: (_) => CreateRequestScreen())); setState(() {}); })
          : _tab == 2
          ? FloatingActionButton(backgroundColor: Colors.green, child: Icon(Icons.sell_outlined, color: Colors.white),
          onPressed: () async { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; } await Navigator.push(context, MaterialPageRoute(builder: (_) => CreateListingScreen())); setState(() {}); })
          : _tab == 3
          ? FloatingActionButton(backgroundColor: Color(0xFFD4A89A), child: Icon(Icons.edit, color: Color(0xFF1A0F0C)),
          onPressed: () { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; } Navigator.push(context, MaterialPageRoute(builder: (_) => NewPrivateChatScreen())); })
          : null,
    );
  }
}
  ...
  Widget _requestCard(Map<String, dynamic> r, bool isOwn) {
    return Card(
      ...
      child: Padding(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          CircleAvatar(
            backgroundColor: Color(0xFF7B2FBE).withValues(alpha: 0.12),
            radius: 22,
            child: Text(
              r['person'].toString().isNotEmpty ? r['person'].toString()[0].toUpperCase() : '?',
              style: TextStyle(color: Color(0xFFD4A89A), fontWeight: FontWeight.bold, fontSize: 18),
            ),
          ),
          SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 8, children: [
          ...
          _chip(Icons.category_outlined, r['category'].toString(), Color(0xFFD4A89A)),
          if ((r['notes']?.toString() ?? '').isNotEmpty)
        ...
        if (isOwn) ...[
          ...
          Container(
            ...
            child: Text('Your request — waiting for someone to lend', style: TextStyle(color: Colors.orange[200], fontSize: 12)),
          ),
        ],
      ])),
    );
  ...
  void _confirmLend(Map<String, dynamic> r) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(height: 10),
        Container(padding: EdgeInsets.all(12),
          decoration: BoxDecoration(color: Color(0xFFD4A89A).withValues(alpha: 0.06), borderRadius: BorderRadius.circular(10)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(r['item'].toString(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
            SizedBox(height: 4),
            Text('To: ${r['person']}  (${r['room']})', style: TextStyle(color: Colors.white60)),
            Text('Duration: ${r['duration']}', style: TextStyle(color: Colors.white60)),
          ]),
        ),
      ]),
      actions: [
        ...
        ElevatedButton(
          onPressed: () async {
            ...
            // NEW: this used to just be a snackbar promise with nothing
            // behind it. Now it actually writes a persisted notification
            // for the borrower, keyed by their email.
            final posterEmail = r['posterEmail']?.toString() ?? '';
            if (posterEmail.isNotEmpty) {
              await FirebaseService.addNotification(
                forEmail: posterEmail,
                message: '${AppData.currentUserName} (${AppData.currentUserRoom}) is lending you the ${r['item']}. Check chat to coordinate pickup.',
              );
            }
            if (!mounted) return;
...
class _MyRequestsPageState extends State<MyRequestsPage> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      TabBar(
        controller: _t,
        labelColor: Color(0xFFCC88FF),
        unselectedLabelColor: Colors.white38,
        indicatorColor: Color(0xFF7B2FBE),
          ...
          itemBuilder: (ctx, i) {
            ...
            return Card(
              ...
              child: Padding(padding: EdgeInsets.all(16), child: Row(children: [
                CircleAvatar(
                  backgroundColor: taken ? Colors.green.withValues(alpha: 0.12) : Color(0xFFD4A89A).withValues(alpha: 0.1),
                  radius: 22,
                  child: Icon(taken ? Icons.check_circle_outline : Icons.hourglass_top_outlined,
                      color: taken ? Colors.green : Color(0xFFD4A89A), size: 22),
                ),
                SizedBox(width: 14),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(r['item'].toString(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                  ...
                  Container(
                    ...
                    child: taken
                      ...
                      Text('✅ Will be lent by:', style: TextStyle(color: Colors.green[300], fontSize: 12, fontWeight: FontWeight.bold)),
                      SizedBox(height: 2),
                      Text('👤 ${r['lenderName'] ?? 'A hostelmate'}', style: TextStyle(color: Colors.green[200], fontSize: 13, fontWeight: FontWeight.w600)),
                      Text('🚪 ${r['lenderRoom'] ?? ''}', style: TextStyle(color: Colors.green[300], fontSize: 12)),
                    ])
                        : Text('⏳ Waiting for a lender', style: TextStyle(color: Colors.orange[200], fontSize: 12)),
                  ),
                ])),
                IconButton(
                  icon: Icon(Icons.delete_outline, color: Colors.red[300], size: 22),
                  onPressed: () async { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; }
                  await FirebaseService.deleteBorrowRequest(r['docId']); },
                ),
              ])),
            );
              ...
              child: Padding(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    SizedBox(height: 3),
                    Text('Lent to: ${l['lentTo']}  (${l['room']})', style: TextStyle(color: Colors.white60, fontSize: 13)),
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(children: [
      ...
      TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => EditProfileScreen())).then((_) => setState(() {})),
          child: Text('Change profile picture', style: TextStyle(color: Color(0xFFD4A89A), fontSize: 13))),
      ...
      Text(AppData.selectedHostelName, style: TextStyle(color: Color(0xFFD4A89A), fontSize: 13)),
      SizedBox(height: 24),
      _m(Icons.edit_outlined, 'Edit Profile', () => Navigator.push(context, MaterialPageRoute(builder: (_) => EditProfileScreen())).then((_) => setState(() {}))),
      _m(Icons.help_outline, 'Help & Support', () => showDialog(context: context, builder: (_) => AlertDialog(title: Text('Support'), content: Text('Email: hostellink@support.com'), actions: [ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C)), child: Text('Close'))]))),
      SizedBox(height: 16),
      SizedBox(width: double.infinity, child: OutlinedButton.icon(
  ...
  Widget _m(IconData i, String t, VoidCallback f) => Card(margin: EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: ListTile(leading: Icon(i, color: Color(0xFFD4A89A)), title: Text(t), trailing: Icon(Icons.arrow_forward_ios, size: 16, color: Colors.grey), onTap: f));
}
...
class _CreateRequestScreenState extends State<CreateRequestScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Post a Request'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C),
              padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          child: _loading ? CircularProgressIndicator(color: Color(0xFF1A0F0C)) : Text('Post Request', style: TextStyle(fontSize: 17)),
        )),
      ])),
    );
  }
...
class _MarketplacePageState extends State<MarketplacePage> {
  ...
  Widget _listingCard(Map<String, dynamic> l) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
      elevation: 0,
      color: isSold ? Color(0xFF15151F) : Color(0xFF1A1A2E),
      child: Padding(padding: EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // Top row
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(l['title'].toString(),
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: isSold ? Colors.white38 : Colors.white)),
            SizedBox(height: 3),
            Text('PKR ${l['price']}',
                style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold, color: isSold ? Colors.white38 : Colors.greenAccent)),
            SizedBox(height: 3),
            Row(children: [
              ...
              if (isSold) Container(padding: EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                  ...
                  child: Text('SOLD', style: TextStyle(color: Colors.white54, fontSize: 11, fontWeight: FontWeight.bold))),
            ]),
          ])),
        ]),
        SizedBox(height: 8),
        Text(l['description'].toString(), style: TextStyle(color: Colors.white70, fontSize: 13)),
        SizedBox(height: 6),
        ...
        if (!isSold) ...[
          ...
          if (isOwn) Row(children: [
            Expanded(child: ElevatedButton.icon(
              onPressed: AppData.isDemoAccount ? () => showDemoBlockedMessage(context) : () => _markSoldDialog(l),
            ...
            SizedBox(width: 10),
            OutlinedButton.icon(
              onPressed: () async { if (AppData.isDemoAccount) { showDemoBlockedMessage(context); return; } await FirebaseService.deleteListing(l['docId']); },
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(padding: EdgeInsets.all(10),
            decoration: BoxDecoration(color: Colors.green.withValues(alpha: 0.07), borderRadius: BorderRadius.circular(10)),
            child: Text('Visit her room or email her to discuss the purchase.', style: TextStyle(color: Colors.green[200], fontSize: 12))),
      ]),
      actions: [ElevatedButton(onPressed: () => Navigator.pop(context),
...
class _CreateListingScreenState extends State<CreateListingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Sell an Item'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white,
      ...
      body: SingleChildScrollView(padding: EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 8, runSpacing: 8, children: _emojis.map((e) {
          ...
          return GestureDetector(onTap: () => setState(() => _emoji = e),
              child: Container(width: 44, height: 44,
                  decoration: BoxDecoration(color: sel ? Colors.green.withValues(alpha: 0.15) : Colors.white.withValues(alpha: 0.06),
        ...
        SizedBox(height: 14),
        Row(children: [
          Expanded(child: DropdownButtonFormField<String>(initialValue: _category, decoration: InputDecoration(labelText: 'Category', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14)),
              items: _categories.map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(), onChanged: (v) => setState(() => _category = v!))),
          SizedBox(width: 12),
          Expanded(child: DropdownButtonFormField<String>(initialValue: _condition, decoration: InputDecoration(labelText: 'Condition', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), contentPadding: EdgeInsets.symmetric(horizontal: 12, vertical: 14)),
        ...
        SizedBox(width: double.infinity, child: ElevatedButton(
          onPressed: _loading ? null : _submit,
          style: ElevatedButton.styleFrom(backgroundColor: Color(0xFFD4A89A), foregroundColor: Color(0xFF1A0F0C), padding: EdgeInsets.symmetric(vertical: 15), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
          child: _loading ? CircularProgressIndicator(color: Color(0xFF1A0F0C)) : Text('Post Listing', style: TextStyle(fontSize: 17)),
        )),
      ])),
    );
  }
...
class _ChatListPageState extends State<ChatListPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: EdgeInsets.all(16), children: [
      SizedBox(height: 8),
      Text('Chat', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFFD4A89A))),
      SizedBox(height: 4),
      ...
      Card(
        ...
        child: ListTile(
          ...
          leading: Stack(clipBehavior: Clip.none, children: [
            CircleAvatar(radius: 26, backgroundColor: Color(0xFFD4A89A),
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        TextButton.icon(
          ...
          icon: Icon(Icons.add, size: 18, color: Color(0xFFD4A89A)),
          label: Text('New', style: TextStyle(color: Color(0xFFD4A89A))),
        ),
      ]),
      SizedBox(height: 8),
            ...
            return Card(
              ...
              child: ListTile(
                ...
                trailing: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  if (unread > 0) ...[
                    SizedBox(height: 4),
                    Container(padding: EdgeInsets.all(5), decoration: BoxDecoration(color: Color(0xFFD4A89A), shape: BoxShape.circle),
...
class _WardenChatTabState extends State<WardenChatTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: EdgeInsets.all(16), children: [
      SizedBox(height: 8),
      Text('Chat', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFFD4A89A))),
      SizedBox(height: 4),
      ...
      SizedBox(height: 20),
      Row(children: [
        Icon(Icons.lock_person_outlined, size: 16, color: Color(0xFFD4A89A)), SizedBox(width: 6),
        Text('Private Messages', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
      ]),
      SizedBox(height: 4),
          ...
          builder: (ctx, snap) {
            ...
            if (docs.isEmpty)
              return Container(
                  ...
                  decoration: BoxDecoration(color: Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(12), border: Border.all(color: Color(0xFF2A2A3E)!)),
                  child: Column(children: [
                    Icon(Icons.chat_bubble_outline, size: 40, color: Colors.white24),
                    SizedBox(height: 8),
            ...
            return Column(children: docs.map((doc) {
              ...
              return Card(
                ...
                child: ListTile(
                  leading: Stack(clipBehavior: Clip.none, children: [
                    CircleAvatar(backgroundColor: Color(0xFFD4A89A),
                        child: Text(otherName.isNotEmpty ? otherName[0].toUpperCase() : '?',
                            style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
                  ...
                  subtitle: Text(lastMsg, maxLines: 1, overflow: TextOverflow.ellipsis,
                      style: TextStyle(fontSize: 12, fontWeight: unread > 0 ? FontWeight.bold : FontWeight.normal, color: unread > 0 ? Colors.white70 : Colors.grey)),
...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        // Info banner
        Container(color: Color(0xFFD4A89A).withValues(alpha: 0.06), padding: EdgeInsets.symmetric(horizontal: 16, vertical: 8),
            child: Row(children: [
...
class _NewPrivateChatScreenState extends State<NewPrivateChatScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('New Message'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white),
      body: _loading
          ? Center(child: CircularProgressIndicator(color: Color(0xFFD4A89A)))
          : _students.isEmpty
            ...
            return ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                return _ChatBubble(
                  ...
                  canDelete: canDelete,
                  onDelete: AppData.isDemoAccount ? () => showDemoBlockedMessage(context) : () async => await FirebaseService.deletePrivateMessage(widget.chatId, docs[i].id),
                );
              },
            );
          },
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: isMe ? CrossAxisAlignment.end : CrossAxisAlignment.start, children: [
            if (!isMe) Padding(padding: EdgeInsets.only(left: 12, bottom: 2), child: Row(mainAxisSize: MainAxisSize.min, children: [
              Text(senderName, style: TextStyle(fontSize: 11, color: Colors.white54, fontWeight: FontWeight.w600)),
              if (isWarden) ...[SizedBox(width: 4), Container(padding: EdgeInsets.symmetric(horizontal: 5, vertical: 1), decoration: BoxDecoration(color: Color(0xFFD4A89A), borderRadius: BorderRadius.circular(8)), child: Text('Warden', style: TextStyle(color: Colors.white, fontSize: 9)))],
            ])),
            Container(
              padding: EdgeInsets.symmetric(horizontal: 14, vertical: 10),
              decoration: BoxDecoration(
                color: isMe ? Color(0xFFD4A89A) : (isWarden ? Color(0xFF9E6B70).withValues(alpha: 0.18) : Color(0xFF1A1A2E)),
                borderRadius: BorderRadius.only(
                  topLeft: Radius.circular(18), topRight: Radius.circular(18),
                  bottomLeft: Radius.circular(isMe ? 18 : 4),
                  bottomRight: Radius.circular(isMe ? 4 : 18),
                ),
                border: isWarden && !isMe ? Border.all(color: Color(0xFF9E6B70).withValues(alpha: 0.3)) : null,
              ),
              child: Text(text, style: TextStyle(color: isMe ? Color(0xFF1A0F0C) : Colors.white, fontSize: 14)),
            ),
...
class _ChatInputBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Color(0xFF1A1A2E), border: Border(top: BorderSide(color: Colors.white12))),
      child: Row(children: [
        ...
        GestureDetector(
          ...
          child: Container(
            padding: EdgeInsets.all(12),
            decoration: BoxDecoration(color: Color(0xFFD4A89A), shape: BoxShape.circle),
...
class NotificationsScreen extends StatelessWidget {
  // NEW: notifications are now real, persisted, per-user data pulled from
  // Firestore (keyed by email) instead of an in-memory list nothing ever
  // wrote into.
  ...
  Widget build(BuildContext context) {
    final email = AppData.currentUserEmail;
    return Scaffold(
      appBar: AppBar(title: Text('Notifications'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white,
          actions: [TextButton(onPressed: () async {
            await FirebaseService.clearAllNotifications(email);
            if (context.mounted) Navigator.pop(context);
          }, child: Text('Clear All', style: TextStyle(color: Colors.white)))]),
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseService.getNotificationsStream(email),
        builder: (ctx, snap) {
          if (snap.connectionState == ConnectionState.waiting) {
            return Center(child: CircularProgressIndicator(color: Color(0xFFD4A89A)));
          }
          final docs = snap.data?.docs ?? [];
          if (docs.isEmpty) {
            return Center(child: Text('No notifications', style: TextStyle(color: Colors.grey)));
          }
          return ListView.builder(
            padding: EdgeInsets.all(16),
            itemCount: docs.length,
            itemBuilder: (ctx, i) {
              ...
              return Dismissible(
                ...
                background: Container(alignment: Alignment.centerRight, padding: EdgeInsets.only(right: 20),
                    decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(12)),
                    child: Icon(Icons.delete_outline, color: Colors.white)),
                onDismissed: (_) => FirebaseService.deleteNotification(email, docs[i].id),
                child: Card(margin: EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                    child: ListTile(leading: CircleAvatar(backgroundColor: Color(0xFF7B2FBE).withValues(alpha: 0.12), child: Icon(Icons.notifications_outlined, color: Color(0xFFD4A89A))),
                        title: Text(data['message']?.toString() ?? ''), subtitle: Text(_formatTime(data['createdAt']), style: TextStyle(color: Colors.grey)))),
              );
            },
          );
        },
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color(0xFF0D0D0D),
      appBar: AppBar(title: Text('Edit Profile'), backgroundColor: Color(0xFF0D0D0D), foregroundColor: Colors.white,
      ...
      body: SingleChildScrollView(padding: EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          padding: EdgeInsets.all(12),
          decoration: BoxDecoration(color: Color(0xFF1A1A2E), borderRadius: BorderRadius.circular(16), border: Border.all(color: Color(0xFF2A2A3E)!)),
          child: GridView.count(
            ...
            children: AppData.avatarEmojis.map((e) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    color: selected ? Color(_color).withValues(alpha: 0.15) : Colors.white.withValues(alpha: 0.06),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(color: selected ? Color(_color) : Colors.white24, width: selected ? 2 : 1),
                  ),
                  child: Center(child: Text(e, style: TextStyle(fontSize: 22))),
                ),
              );
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: selected ? [BoxShadow(color: Color(c).withValues(alpha: 0.6), blurRadius: 8, spreadRadius: 2)] : [],
              ),
