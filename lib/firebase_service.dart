...
class FirebaseService {
  ...
  // Keyed by sanitized email rather than uid, because a student's
  // Firestore "uid" field can change (custom id -> real Firebase Auth uid)
  // on their first login, but their email never does.
  // ═══════════════════════════════════════════════════════════════════
  static String _notifKey(String email) =>
      email.trim().toLowerCase().replaceAll('.', '_').replaceAll('@', '_');

  static CollectionReference _notificationsFor(String email) => _db
      .collection('userNotifications').doc(_notifKey(email)).collection('items');

  /// Call this from whatever action should notify [forEmail] — e.g. a
  /// lending being confirmed, or a registration request being approved.
  static Future<void> addNotification({
    required String forEmail,
    required String message,
  }) async {
    try {
      await _notificationsFor(forEmail).add({
        'message':   message,
        'createdAt': FieldValue.serverTimestamp(),
      });
    } catch (_) {}
  ...
  static Stream<QuerySnapshot> getNotificationsStream(String email) =>
      _notificationsFor(email)
          .orderBy('createdAt', descending: true)
          .limit(50)
          .snapshots();

  static Future<void> deleteNotification(String email, String docId) async {
    try { await _notificationsFor(email).doc(docId).delete(); } catch (_) {}
  }

  static Future<void> clearAllNotifications(String email) async {
    try {
      final snap = await _notificationsFor(email).get();
      final batch = _db.batch();
